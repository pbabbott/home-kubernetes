...
2. **Pre-apply**: from a machine with cluster access, `kubectl apply -f applications/non-prod-gen2/flux-gitops/fluxinstance.yaml` once `crds-ks` has applied the Flux Operator CRDs (or the operator chart has installed them), then push the commit that removes bootstrap manifests.
